...
const registeredClients = new Map(); // dynamic client registration (RFC 7591); addition to the workshop reference for clients without CIMD support, e.g. Claude Code
async function resolveClient(clientId) {
    if (registeredClients.has(clientId)) return registeredClients.get(clientId);
...
function isRegisteredRedirectUri(requested, client) {
    ...
    // RFC 8252 §7.3: loopback redirect URIs match regardless of port (native clients such as Claude Code bind an ephemeral port).
    let url;
    try { url = new URL(requested); } catch { return false; }
    if (url.protocol !== 'http:' || !LOOPBACK_HOSTS.has(url.hostname)) return false;
    url.port = '';
    return registered.includes(url.href);
}
...
export function createOAuthProxy({ issuerUrl, resourceUrl, apsClientId, apsClientSecret, callbackUrl }) {
    ...
    router.post('/register', (req, res) => {
        ...
        const client = { client_id: clientId, client_name: clientName, redirect_uris: redirectUris, token_endpoint_auth_method: 'none', grant_types: ['authorization_code', 'refresh_token'], response_types: ['code'] };
